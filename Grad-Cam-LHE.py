...
def prepare(img_path):
    ...
    #new_array=cv2.resize(img_array,(IMG_SIZE,IMG_SIZE))
    return temp_img_array.reshape(-1,IMG_SIZE,IMG_SIZE,3)
...
model = tf.keras.models.load_model("AlexNet(Right-Disk%s).h5" %Disk) 
...
gary2rgb = cv2.cvtColor(img_eq,cv2.COLOR_GRAY2RGB)
resized = cv2.resize(gary2rgb,(224,224))
img = keras.preprocessing.image.img_to_array(resized)
plt.imshow(gary2rgb)
...
save_path = "Right-LHE-Disk10-Grad-Cam.jpeg"
